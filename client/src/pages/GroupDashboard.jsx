...
import {
    getMyComics as getComics, // Alias to keep name consistent or just use getMyComics
    getChapters,
    createComic,
    updateComic,
    deleteComic,
...
function GroupDashboard() {
    ...
    useEffect(() => {
        if (!authLoading) {
            if (!isAuthenticated) {
                navigate('/login');
            } else if (user?.role !== 'group' && user?.role !== 'admin') {
                navigate('/');
            }
        }
    ...
    useEffect(() => {
        if (isAuthenticated && (user?.role === 'group' || user?.role === 'admin')) {
            fetchComics(1, '');
        }
    }, [isAuthenticated, user, fetchComics]);
    ...
    const handleLogout = () => {
        // Just navigate home, actual logout is in header
        navigate('/');
    };
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <div className="flex items-center justify-between mb-4 bg-white dark:bg-dark-card p-4 rounded-lg shadow-sm border border-gray-100 dark:border-dark-border">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-lg font-bold text-primary">Kênh Nhóm Dịch</h1>
                </div>
                <div className="flex items-center gap-2">
                    <button
